...
void LPInnerProductLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (M_ == 1) {
    ...
    if (bias_term_)
      caffe_gpu_axpy<Dtype>(N_, bias_multiplier_.cpu_data()[0],
                            this->blobs_[bias_loc]->gpu_data(), top_data);
  } else {
    ...
    if (bias_term_)
        caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, 1, (Dtype)1.,
                              ...
                              this->blobs_[bias_loc]->gpu_data(), (Dtype)1., top_data);
  }
}

template <typename Dtype>
void LPInnerProductLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(LPInnerProductLayer);
