...
void LPConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  // Store low-precision parameters
  ...
  // - blobs_[1] holds the rounded filter weights
  // - blobs_[2] holds the biases (optional)
  // - blobs_[3] holds the rounded biases (optional)
  ...
  if (this->blobs_.size() > 0) {
    CHECK_EQ(2 + 2*this->bias_term_, this->blobs_.size())
    ...
    shared_ptr<Filler<Dtype> > weight_filler_rd(GetFiller<Dtype>(
        this->layer_param_.convolution_param().weight_filler()));
    weight_filler_rd->Fill(this->blobs_[1].get());
    ...
    if (this->bias_term_) {
      ...
      shared_ptr<Filler<Dtype> > bias_filler_rd(GetFiller<Dtype>(
          this->layer_param_.convolution_param().bias_filler()));
      bias_filler_rd->Fill(this->blobs_[3].get());
    }
  }
...
template <typename Dtype>
void LPConvolutionLayer<Dtype>::compute_output_shape() {
...
// void LPConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
//       const vector<Blob<Dtype>*>& top) {
//   caffe_cpu_round_fp(this->blobs_[0]->count(), BD_, AD_,
...
//         const Dtype* bias;
//         if (this->round_bias_){
//           caffe_cpu_round_fp(this->blobs_[2]->count(), BD_, AD_,
...
template <typename Dtype>
void LPConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(LPConvolutionLayer);
#endif

INSTANTIATE_CLASS(LPConvolutionLayer);
