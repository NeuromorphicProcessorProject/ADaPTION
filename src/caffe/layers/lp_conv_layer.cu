...
void LPConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  caffe_gpu_round_fp(this->blobs_[0+1]->count(), this->BD_, this->AD_, this->rounding_scheme_,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < this->num_; ++n) {
      ...
      if (this->bias_term_) {
        const Dtype* bias;
        if (this->round_bias_){
          caffe_gpu_round_fp(this->blobs_[2+1]->count(), this->BD_, this->AD_, this->rounding_scheme_,
